package io.prophecy.pipelines.automationpbtscalafalse11
...
import io.prophecy.pipelines.automationpbtscalafalse11.graph._
...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri",
                   "pipelines/AutomationPBT-scalafalse11"
    )
    registerUDFs(spark)
    MetricsCollector.instrument(spark, "pipelines/AutomationPBT-scalafalse11") {
      apply(context)
    }
  }
